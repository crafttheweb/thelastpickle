...
The first step in understanding the 3.x storage engine is to accept the new mental model. For previous versions of Cassandra, those that knew the Thrift API basically knew how the storage engine worked; each internal _row_ was an ordered list of _columns_ that optionally had a value. This was kind of fun to start with, then we started having more complicated use cases which lead to some common abstractions. These abstractions were formalised in CQL3, which gave birth to terms such as _Partition_ and _Clustering_. Though useful conceptually, they were not natively supported by the old storage engine.

The 3.x storage engine however, does natively supports the Partition and Clustering concepts of CQL3. In both old and new storage engines, a Partition is a collection of Rows sharing the same _Partition Key(s)_ that are ordered within the Partition by their _Clustering Key(s)_. Rows are then by globally identified by the _Primary Key_: the combination of Partition Key and Clustering Key.

The important change is that the 3.x storage engine now knows about these ideas. It may seem strange, but the older storage did not actually know about the Rows in a Partition as it was structured to deal with the Thrift model. The new storage engine is implemented specifically to handle CQL3 the abstractions in a way that facilitates increases in throughput while lowering storage requirements for the same amount of data.

With this new structure in mind, it's time to talk about internals. 
